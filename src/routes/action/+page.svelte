<script lang="ts">
    ...
    import {
        pack,
        reps as storeReps,
        issues as storeIssues,
        recordAction,
        recordCombinedScript,
    } from "$lib/store";
    ...
    onMount(() => {
        ...
        for (const action of $pack.actions) {
            if (action.script) {
                restored[
                    actionKey(action.rep_id, action.issue_id, action.channel)
                ] = {
                    loading: false,
                    text: action.script,
                    error: "",
        ...
        const restoredCombined: Record<string, ScriptEntry> = {};
        for (const [key, script] of Object.entries(
            $pack.combined_scripts ?? {},
        )) {
            restoredCombined[`combined:${key}`] = {
                loading: false,
                text: script,
                error: "",
            };
        }
        combinedScripts = restoredCombined;
    });
    ...
    async function generateScript(
        ...
        if (template) {
            ...
            if (existing)
                recordAction({ ...existing, script_used: true, script: text });
            return;
        }
    ...
    async function generateCombinedScript(
        rep: Representative,
        channel: string,
    ) {
        ...
        try {
            ...
            recordCombinedScript(rep.id, channel, data.script);
        } catch (e) {
            combinedScripts[ck] = {
                loading: false,
                text: "",
    ...
    function contactInfo(
        rep: Representative,
        channel: string,
    ): { href: string | null; display: string } | null {
        switch (channel) {
            case "phone":
                return rep.phone
                    ? { href: `tel:${rep.phone}`, display: rep.phone }
                    : null;
            case "email":
                return rep.email
                    ? { href: `mailto:${rep.email}`, display: rep.email }
                    : null;
            case "web_form":
                return rep.web_form_url
                    ? { href: rep.web_form_url, display: "Form link" }
                    : null;
            case "fax":
    ...
    const completedCount = $derived(
        $pack.actions.filter((a) => a.status === "sent").length,
    );
...
<div class="container" style="padding-top: 2.5rem; padding-bottom: 6rem;">
    <div class="page-header">
            ...
            <p style="color: var(--color-text-muted); margin: 0;">
                ...
                {#if totalSelected > 0}
                    <strong style="color: var(--color-success);"
                        >{completedCount} of {totalSelected} done.</strong
                    >
                {/if}
            </p>
        </div>
        <div class="header-actions">
            ...
            {#if mailActionCount > 0}
                <a href="/print-letters" class="btn btn-secondary"
                    >Print letters</a
                >
            {/if}
            <button
                class="btn btn-secondary"
                onclick={() => (showSaveDialog = true)}>Save pack</button
            >
        </div>
    </div>
    ...
    {#if !activeReps.length || !issues.length}
        <div class="empty-state card">
            ...
            <p>
                Go back to <a href="/packet">select actions</a> to choose which channels
                to use.
            </p>
        </div>
    {:else}
                        ...
                        <tr>
                            <td class="rep-cell">
                                <div class="rep-cell-inner">
                                    ...
                                    <div class="rep-meta">
                                        <span class="badge badge-{rep.level}"
                                            >{LEVEL_LABELS[rep.level]}</span
                                        >
                                        {#if rep.district_name}
                                            <span class="rep-dept"
                                                >{rep.district_name}</span
                                            >
                                        {/if}
                                    </div>
                                </div>
                            ...
                            {#each issues as issue}
                                {@const actions = selectedChannels(
                                    rep.id,
                                    issue.id,
                                )}
                                            ...
                                            <div class="action-main">
                                                ...
                                                {#if entry?.text}
                                                    <div class="script-box">
                                                        <pre
                                                            class="script-text">{entry.text}</pre>
                                                        <button
                                                            class="btn-copy"
                                                            onclick={() =>
                                                                copyScript(
                            ...
                            {/each}
                            {#if issues.length > 1}
                                {@const channels = uniqueChannelsForRep(
                                    rep.id,
                                ).filter(
                                    (ch) =>
                                        issuesForRepChannel(rep.id, ch).length >
                                        1,
                                )}
    ...
    >
        ...
        >
            ...
            <p style="font-size: 0.875rem; color: var(--color-text-muted);">
                Your session is encrypted in your browser and saved as a file on
                your device. The passphrase never leaves your machine.
            </p>
            ...
            {#if saveError}
                <p
                    style="color: var(--color-danger); font-size: 0.875rem; margin-top: 0.5rem;"
                >
                    {saveError}
                </p>
            {/if}
