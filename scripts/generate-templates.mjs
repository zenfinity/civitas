...
function buildPrompt(issue, channel, tone) {
	...
	switch (channel) {
		...
		case 'mail':
			return `Write a formal letter from ${from} to ${rep} about "${issue.label}".${ctx} Write ${toneP}. Format it as a formal letter with this exact structure: first a sender block on three lines: "{{YOUR_NAME}}" then "{{YOUR_ADDRESS}}" then "{{YOUR_EMAIL}}". Then a blank line, then "{{DATE}}", then a blank line, then the salutation "Dear {{REP_TITLE}} {{REP_NAME}},", then 2–3 body paragraphs, then "Sincerely," followed by {{YOUR_NAME}} on the next line. Under 300 words. Plain text only, no markdown. Use {{YOUR_NAME}}, {{YOUR_ADDRESS}}, {{YOUR_EMAIL}}, {{REP_NAME}}, {{REP_TITLE}}, {{DISTRICT}}, {{DATE}} as literal placeholders — do not substitute them.`;
		case 'web_form':
